...
func TestCompress(t *testing.T) {
	dictionarySizes := []DictionarySize{
		XS,
		S,
		M,
		L,
	...
	for _, testCase := range compressTestCases {
		for _, dictionarySize := range dictionarySizes {
			...
			actual, err := CompressWithDictSize(bv, dictionarySize)
			...
			if actual.MustGet(0) != uint16(dictionarySize) {
				t.Error("Expected size to be encoded into compressed bytes")
			}

			for i := 0; i < testCase.expectedOutput.Size(); i++ {
				if testCase.expectedOutput.MustGet(i) != actual.MustGet(i+1) {
...
var decompressTestCases = []struct {
	...
	{
		name:  "Hello world",
		input: vector.New().AppendToCopy(uint16(XL), uint16(72), uint16(101), uint16(108), uint16(108), uint16(111), uint16(32), uint16(87), uint16(111), uint16(114), uint16(108), uint16(100)),
...
func TestDecompressedEqualsOriginal(t *testing.T) {
	...
	for _, dictionarySize := range dictionarySizes {
		...
		compressed, err := CompressWithDictSize(origVector, dictionarySize)
		if err != nil {
